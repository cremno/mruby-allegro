MRuby::Gem::Specification.new('mruby-allegro') do |spec|
  ...
  if ENV['OS'] == 'Windows_NT'
    spec.linker.libraries << %w(allegro-5.0.8-monolith-static-mt-debug freetype-2.4.8-static-mt-debug user32 opengl32 gdi32 winmm ole32 shell32 shlwapi gdiplus comdlg32)
  else
    spec.cc.include_paths << ['/usr/local/include']
    spec.linker.library_paths << ['/usr/local/lib']
    spec.linker.libraries << ['allegro', 'allegro_primitives', 'allegro_dialog', 'allegro_image', 'allegro_font', 'allegro_ttf']
  end
